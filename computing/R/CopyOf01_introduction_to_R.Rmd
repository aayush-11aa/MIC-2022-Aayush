...
v <- c(1:4)
M1 <- matrix(v, ncol = 2, nrow = 2)
print(M1)
w <- seq(from = 1, to = 24, by = 1)
M2 <- matrix (w, ncol = 6, nrow = 4)
print(M2)
r <- seq(from =1, to =4, by =1)
M3 <- cbind(M2, r)
print(M3)
...
print (M)
identity_matrix <- diag(x=1,nrow=4,ncol=4)
...
```{r}
my_func_aa <- function(a) {
    b <- a * 3
    return(b)
}
my_func_aa2 <- function(v,w) {
    r <- v+w
    return(r)
}
```
...
# gives you the first element and its name 'first'
```{r}
my_list[1]
```

#gives the value in the 'second' list
```{r}
my_list["second"]
```
#gives only the value
```{r}
my_list$third
```
# double big bracket gives just the value
```{r}
my_list[["third"]]
```
...
hist(mtcars$wt, breaks = 30) # breaks- how many bins to create
```
```
