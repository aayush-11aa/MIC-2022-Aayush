...
iris %>% sample_n(10) # 10 random samples

iris %>% sample_n(10) %>% .$Petal.Width %>% mean() #dot represents to pull numbers from 'iris'
```
...
```
# currently the table aove is in wide format
### Pivot_longer
# we will put in long format
...
df %>% 
pivot_longer(cols = c(visit1,visit2), 
                 names_to = "visit", values_to = "measurement") %>%  # need to repeat this step from previous chunk since we didnt make permanent changes to df
...
pivot_longer(cols = c(visit1,visit2), 
                 names_to = "visit", values_to = "measurement") %>%
separate(desc, sep='-', into=c("sex", "age")) %>%
mutate(age=as.integer(age),  #rewrites the column age as integers
       visit=str_remove(visit, "visit"),
       visit=as.integer(visit)) %>%
drop_na(measurement) -> df1 #NA is a special keyword in R- its not a string. its put there by R when the value is missing
...
inner_join(df, names, by = "pid") # shows only the stuff that matches in both tibbles

left_join(df, names, by = "pid") # does the same thing; prioritize df as the standard and everything from here will be kept

right_join(df, names, by = "pid") # where there is missing match then show na; prioritize names as the standard

full_join(df, names, by = "pid") # combines all
```
