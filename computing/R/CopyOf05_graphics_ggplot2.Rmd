...
```{r}
n <- 100
x <- sort(runif(n)) # randomly selecting numbers between 0 and 1
...
g2 <- g1 +
      geom_smooth(method='lm', 
              formula=y ~ poly(x, 2, raw=TRUE)) # this is a polynomial fit (quadratic because it's degree 2) #method='lm' means we want linear model
...
g3 <- g2 +
      geom_point(aes(color = z)) #aes is mapping between dataset and color: becasuse z is continuous so its giving a range of colors
...
```{r}
g4 + theme_linedraw() # puts a box around the graph
```
...
```{r}
g4 + facet_grid(g ~ .) # rotates the above plot
```
...
```{r}
g0 + geom_line() # line plot; geom_line doesnt do a fit. It just connect points
```


```{r}
ggplot(df, aes(x=z, fill=g)) + #fill=g (categorical variable)
geom_density(aes(alpha=0.1)) + #density= how dark of a color
guides(alpha=F) #alpha is just density of color that looks the best
```
...
g0 + 
geom_density_2d() + 
geom_rug() # rug shows the actual data points
```
...
geom_boxplot() +
geom_jitter(width=0.2)  + #jitter- defines distance between points
guides(fill=FALSE)
```
